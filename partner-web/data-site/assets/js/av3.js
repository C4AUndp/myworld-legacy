...
// var partnerlist = api1 + "partner_rankings";
...
var getId = function (p) {
	...
	if (p.country) {
		...
		var xapi = overview + "?country=" + m[0].id;
    var tapi = api1 + "tseries/" + daysUrl(today) + "/" + daysUrl(lastWeek) + "?country=" + m[0].id;
		// $("select#s-country")[0].selectedIndex = parseInt(m[0].id) + 35 ;
    $("select#s-country").val("&country=" + parseInt(m[0].id));

    callAPI(xapi)
		
		var sapi = api2 + "segment?country=" + m[0].id;
		callSegment(sapi);
		// calltSeries(tapi)
	...
	// For Partner Parameters
		...
		var xapi = overview + "?partner=" + p.partner;
    var capi = api2 + "countries" + "?partner=" + p.partner;
		callAPI(xapi)
		
		var sapi = api2 + "segment?partner=" + p.partner;
		callSegment(sapi);
    callCounties(capi)
	...
	// For Source Parameters
		...
		var xapi = overview + "?sourceMethod=" + m[0].id;
    var capi = api2 + "countries" + "?sourceMethod=" + m[0].id;

		callAPI(xapi)

		var sapi = api2 + "segment?sourceMethod=" + m[0].id;
		callSegment(sapi);
	...
	// All others & unknown call direct API
		...
		var capi = api2 + "countries";
    var sapi = api2 + "segment?";
		var tapi = api1 + "tseries/" + daysUrl(today) + "/" + daysUrl(lastWeek);
		callAPI(overview)
		calltSeries(tapi);
		callSegment(sapi);
    callCounties(capi);
  ...
  tTotal.selectAll(".tTotalcircle")
    ...
    // .style("stroke", "#ccc")
          .call(d3.helper.tooltip(function(d, i){
            var tcolor = "#333";
            return "<div style='text-align:center;color:#fff;background:"+tcolor+"'><span class='b'>Total Ballots: </span>"+coma(d.value)+"</div>";
          }));
    ...
    .style("fill", "#000")
    .style("opacity", .4)
    .text(function(d) { return coma(d.value)})
...
function redraw() {
  ...
  ldata = ldata.filter(function(d){ return d.pri_id > 0})

  ldata.sort(function(obj1, obj2) {
    return obj2.count - obj1.count;
  });
  // ldata = d3.key(function(d) { return d.count; }).sortKeys(d3.ascending)


	y.domain(ldata.map(function(d) { return d.pri_id; }));
	...
	var bar = svg.selectAll(".bar")
      .data(ldata, function(d) { return d.pri_id; });

     var barEnter = bar.enter().insert("g", ".axis")
      .attr("class", "bar")
      .attr("transform", function(d) { return "translate(0," + (y(d.pri_id) + height) + ")"; })
      .style("fill-opacity", 0);
      ...
      .style("fill", function(d) { 
        return priorities[d.pri_id]['item-color']; }) 
      ...
      .attr("text-anchor", "end")
      .text(function(d) {
        return priorities[d.pri_id]['item-title']; })
	...
	var barUpdate = d3.transition(bar)
      .attr("transform", function(d) { return "translate(0," + (d.y0 = y(d.pri_id)) + ")"; })
      .style("fill-opacity", 1);


  barUpdate.select(".value")
      .attr("x", function(d) { return x(d.count) + 3; })
      .text(function(d) { return coma(d.count); });
...
function change () {
    ...
    var sapi = api+s_country+s_gender+s_edu+s_age;

		// if (ageVal[0]==1 && ageVal[1] == 140){
		// 	var sapi = api+s_country+s_gender+s_edu;
  //     var csv = "http://ec2-54-242-123-146.compute-1.amazonaws.com/stats/csv/segment?" +s_country+s_gender+s_edu
		// }

		// else { 
  //     var sapi = api+s_country+s_gender+s_edu+s_age1+s_age2; 
  //     var csv = "http://ec2-54-242-123-146.compute-1.amazonaws.com/stats/csv/segment?"+s_country+s_gender+s_edu+s_age1+s_age2
    // }
    ...
    var title = $('select#s-country option:selected').text() + " / " + $('select#s-gender option:selected').text() + " / " + $('select#s-edu option:selected').text() + " / " + "Age Group ("+ $('select#s-age option:selected').text() + ")"
		$("#segmentv1").text(coma(ndata._total))
...
var segmentSummary = function (data) {
	...
	var margin = {top: 90, right: 0, bottom: 60, left: 280},
		width = 940 - margin.left - margin.right,
...
var cTable = function (data) {
  ...
  data.countries.sort(function(obj1, obj2) {
    return obj2.stats._total - obj1.stats._total;
  });

	var hdi_title = ["Low","Medium","High","Very High"]
	var table_obj = $('#countrylist');
    $.each(data.countries, function(index, item){
      var ttotal = data.overall._total;
      var ctotal = item.stats._total;
    	// var edut =  item.demo.education[1] + item.demo.education[2] + item.demo.education[3] + item.demo.education[4];

    	var c = '<td style="text-align:left;">'+item.name+'</td>'
    	var hdi = '<td>'+hdi_title[item.hdi_index-1]+'</td>'
    	var t = '<td>'+ctotal+'</td>'
    	var tp = '<td>'+(ctotal/ttotal*100).toFixed(1)+'%</td>'
    	var m = '<td>'+Math.round(item.stats.dimensions.gender['1']/ctotal*100)+'%</td>'
    	var f = '<td>'+Math.round(item.stats.dimensions.gender['2']/ctotal*100)+'%</td>'
    	var a1 = '<td>'+Math.round(item.stats.dimensions.age['15']/ctotal*100)+'%</td>'
    	var a2 = '<td>'+Math.round(item.stats.dimensions.age['30']/ctotal*100)+'%</td>'
    	var a3 = '<td>'+Math.round(item.stats.dimensions.age['45']/ctotal*100)+'%</td>'
      var a4 = '<td>'+Math.round(item.stats.dimensions.age['60']/ctotal*100)+'%</td>'
      var a5 = '<td>'+Math.round(item.stats.dimensions.age['140']/ctotal*100)+'%</td>'
    	var e1 = '<td>'+Math.round(item.stats.dimensions.education['1']/ctotal*100)+'%</td>'
    	var e2 = '<td>'+Math.round(item.stats.dimensions.education['2']/ctotal*100)+'%</td>'
    	var e3 = '<td>'+Math.round(item.stats.dimensions.education['3']/ctotal*100)+'%</td>'
    	var e4 = '<td>'+Math.round(item.stats.dimensions.education['4']/ctotal*100)+'%</td>'

		table_obj.append($('<tr id="'+item.name+'">'+c+hdi+t+tp+m+f+a1+a2+a3+a4+a5+e1+e2+e3+e4+'</tr>'));
...
$.getJSON(list, function(data) {
    ...
    partners.sort(function(obj1, obj2) {
      return obj2.tVoters - obj1.tVoters;
    });
        ...
        // $("ul#drop-partner").append($('<li><a tabindex="-1" href="?partner='+item.volunteer_id+'">'+item.organization+'</a></li>'));
