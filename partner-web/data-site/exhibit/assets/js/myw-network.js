...
var networkFileLocation = "assets/data/new_network_3.json";
...
d3.json(networkFileLocation, function(raw) {
  // console.log(raw)
  var c=0;
  for (key in raw) {
    country.push({name: key, cid: c})
    c++;
  }

  for (key in raw) {
    var source;
    country.forEach(function (d) { if (key === d.name) {source = d.cid} });

    for (link in raw[key]) {
      var target;
      country.forEach(function (d) { if (link === d.name) {target = d.cid} });
      links.push({source: source, target: target, value: parseFloat(raw[key][link])});
    }
  }
  renderNetwork();
});
...
var radius = d3.scale.sqrt().range([1, 6]);

 links = links.filter(function(d, i) { return d.value > 0.6; });
...
function resize() {
  ...
  widthF = elem.innerWidth()-marginW, heightF = 800;
