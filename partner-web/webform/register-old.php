...
if(isset($_POST['submit'])){
    ...
    if($row||empty($_POST['s'])||empty($_POST['email'])|| empty($_POST['email2'])|| $_POST['email'] != $_POST['email2'] ||empty($_POST['p1'])){
        ...
        $query = sprintf("INSERT INTO volunteers(`volunteer_id`,`first_name`,`last_name`,`email`,`password`,`organization`,`country_code`,`phone_number`,`country`,`state`,`admin`)
            VALUES('$usr','%s','%s','%s','%s','%s','%s','%s','%s','%s','0')",
		...
		$password=$_POST['p1'];
		 ...
		 $from = "MyWorld Ballot Entry System <xavier.longan@undp.org>";
		 $to = "$name <$email>";
		 $subject="MyWorld Ballot Entry System - MY World Partner Login Information";
		 $body="<span style='font-size: 14px;'>
			$name,<br><br>
			Welcome to MY world partner site for offline vote collection.<br>
			<br>
			...
			<br>
			<b>MY World Partner ID: $username<br>
			Password: $password</b><br>
			<br>
			MyWorld Ballot Entry System<br><br>
			If you have any questions or problems accessing your account please email the MY World partner focal point (xavier.longan@undp.org).
			</span>";
		...
		?>
		<script>alert ('Thank you for registering!  You will receive an email with your Partner ID and password.'); window.location = 'index.php'</script><?php
		//header("Location: admin.php");
        exit;
    }
}
...
<head>
	<meta charset="UTF-8">
	<title>Ballot Entry Form - Partner Registration</title>
